...
export default function LoginPage() {
  return (
    <div className="login-page">
      <header className="login-header"><Link href="/" aria-label="Aurora Hotel, về trang chủ">AURORA HOTEL</Link><span>Guest access</span></header>
      <main className="login-main"><div className="login-panel"><p className="login-eyebrow">Aurora · guest account</p><h1>Trở lại<br /><em>với kỳ nghỉ.</em></h1><p className="login-lede">Đăng nhập để xem hồ sơ đã xác thực. Nếu chỉ cần kiểm tra một booking, bạn có thể dùng tra cứu bằng mã và email.</p><LoginForm /><Link href="/my-bookings" className="login-lookup-link">Tra cứu không cần đăng nhập ↗</Link></div></main>
      <style>{`
        ...
        .login-header a { font: 600 24px/1 var(--font-display); letter-spacing: .1em; }
        .login-header span { color: var(--antique-brass); font-size: 9px; font-weight: 700; letter-spacing: .14em; text-transform: uppercase; }
        ...
        .login-eyebrow { margin: 0 0 14px; color: var(--muted-terracotta); font-size: 9px; font-weight: 700; letter-spacing: .2em; text-transform: uppercase; }
        ...
        .login-lede { margin: 24px 0 0; color: var(--taupe); font-size: 13px; line-height: 1.8; }
        .login-form { display: grid; gap: 15px; margin-top: 28px; }
        .login-form label { display: grid; gap: 8px; color: var(--taupe); font-size: 9px; font-weight: 700; letter-spacing: .12em; text-transform: uppercase; }
